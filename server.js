...
async function handleNewPostWithPersistence(socket, data) {
    ...
    // 🎯 SOLO aplicar límite a posts generales, no a posts de DJs
    const isDJPost = data.postType === 'dj' ||
                          data.content.includes('🎧 MIX:') ||
                          data.content.includes('🎛️ SET:') ||
                          data.content.includes('🤝 COLABORACIÓN:') ||
                          data.content.includes('📅 EVENTO:') ||
                          data.content.includes('💿 PROYECTO:') ||
                          data.content.includes('🔍 BUSCO:') ||
                          data.content.includes('🎵 TRACK:') ||
                          data.content.includes('🎚️ EQUIPO:');
    ...
    if (!isDJPost) {
        ...
        const userPostedToday = state.posts.some(post =>
            ...
            // Solo contar posts generales, no de DJs
            !(post.content.includes('🎧 MIX:') ||
              post.content.includes('🎛️ SET:') ||
              ...
              post.content.includes('🎚️ EQUIPO:'))
        );

        if (userPostedToday) {
            socket.send(JSON.stringify({
                type: 'error',
                message: '¡Solo 1 publicación general por día! 🌅\nUsa las herramientas de DJ para compartir mixes, sets, eventos y más sin límites 🎧'
            }));
            return;
        }
    }
